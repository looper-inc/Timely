import {
    SafeAreaView, Switch, StyleSheet, ScrollView, TextInput, Image, Platform, Dimensions, TouchableWithoutFeedback, ActivityIndicator
} from 'react-native'
...
const OtherProfileScreen = ({ navigation, route }: any) => {
    ...
    useEffect(() => {
        ...
        db.collection('profiles').doc(route.params.uid).get().then(snapshot => {
            const newProfile = snapshot.data();
            setProfile(newProfile)
            db.collection('profiles').doc(route.params.uid).collection('friend_list').doc(currentUser.uid).get().then(snapshot => {
    ...
    function handleFriend() {
        ...
        db.collection('profiles').doc(route.params.uid).collection('friend_list').doc(currentUser.uid).set({
    ...
    function handleUnfriend() {
        ...
        db.collection('profiles').doc(currentUser.uid).collection('friend_list').doc(route.params.uid).delete()
        db.collection('profiles').doc(route.params.uid).collection('friend_list').doc(currentUser.uid).delete()
        setFriendStatus(1)
    }
    ...
    function renderAddButton() {
        ...
        if (friendStatus === 1) {
            return (
                <FormButton buttonTitle="Add Friend" onPress={handleFriend} />
            )
        }
        if (friendStatus === 2) {
            return (
        ...
        if (friendStatus === 3) {
            return (
                <FormButton buttonTitle="Unfriend" onPress={handleUnfriend} />
            )
        }
    }

    const { email, bio } = profile
    const image = profile.profileImgURL ? { uri: profile.profileImgURL } : require('../assets/images/proplaceholder.jpg')
